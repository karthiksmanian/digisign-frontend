...
export const ShareModal: React.FC<{
  ...
  onClose: () => void;
}> = ({ fileId, fileName, sharedTo, onClose }) => {
  ...
  const handleActiveEmailCheck = (ithEmail: string) => {
    ...
    console.log(ithEmail, user.email);
    return ithEmail == user.email;
  };
  ...
  const updateSharedTo = async (email: string, fileId: string) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/pdfs/share`,
        {
          method: "POST",
          headers: {
  ...
  const handleRemoveShare = async (index: number) => {
    try {
      ...
      if (response) {
        var new_emails = sentForSigning.filter(
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full flex justify-center items-center z-50">
      ...
      <div className="bg-white p-5 rounded-lg shadow-lg z-10">
        ...
        <div className="text-lg bg-gray-800 text-white rounded-md cursor-default py-2 text-center font-semibold mb-2 text-sm">
          Yet to sign
        </div>
        <ul className="text-lg mb-2 text-sm">
          {sentForSigning.map((row, index) => {
            return !handleActiveEmailCheck(row) ? (
              <div className="flex w-full items-center justify-between">
                <li
                  className="text-center w-full py-1 px-4 rounded border m-1 bg-gray-200"
                  key={index}
                >
                  {row}
                </li>
                <div className="mr-1">
                  <FaTrash
                    title="remove access"
                    className="cursor-pointer mx-3 text-red-700 hover:text-red-500"
              ...
              <div className="flex w-full items-center justify-between">
                <li
                  className="text-center w-full py-1 px-4 rounded border m-1 bg-gray-300"
                  key={index}
                >
                  {row} (you)
                </li>
